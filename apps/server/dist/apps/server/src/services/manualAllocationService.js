...
async function resolveTargetTitle(targetType, targetId) {
    try {
        if (targetType === TargetType.PROJECT) {
            ...
            if (s)
                return `${s.project.title} -> ${s.title}`;
            return targetId;
        }
        else {
            ...
            if (c)
                return `${c.subProject.project.title} -> ${c.subProject.title} -> ${c.title}`;
            return targetId;
        }
    }
    catch {
...
async function aggregate(items) {
    const map = new Map();
    for (const item of items) {
        ...
        if (!map.has(key))
            map.set(key, { itemName: item.itemName, unit: item.unit, totalQuantity: 0, breakdown: [] });
...
//# sourceMappingURL=manualAllocationService.js.map