...
router.get("/", asyncHandler(async (req, res) => {
  const { targetType, targetId, aggregated } = req.query as any;
  if (!targetType || !targetId) return res.status(400).json({ success: false, error: "targetType and targetId required" });
  ...
  const data = await manualAllocationService.getForTarget(targetType as TargetType, targetId);
