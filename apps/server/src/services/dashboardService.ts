...
export const dashboardService = {
  async getStats() {
    ...
    const getLeafRevenue = (startD: Date, endD?: Date) => {
      let rev = 0;
      const isCompletedInPeriod = (node: any) => {
      ...
      for (const p of allProjects) {
        if (p.subProjects.length === 0) {
          if (isCompletedInPeriod(p)) rev += p.value.toNumber();
        } else {
          for (const sub of p.subProjects) {
            if (sub.children.length === 0) {
              if (isCompletedInPeriod(sub)) rev += sub.value.toNumber();
            } else {
              for (const child of sub.children) {
                if (isCompletedInPeriod(child)) rev += child.value.toNumber();
              }
            }
          }
        }
