...
import { CreateLedgerEntryDTO, UpdateLedgerEntryDTO } from '@sg/types';
...
async function calculateAllocations(
  ...
  tx: TxClient
): Promise<{ targetType: TargetType, targetId: string, amount: Prisma.Decimal }[]> {
  ...
  const resolvedTargets = await Promise.all(targets.map(async t => {
    if (t.targetType === TargetType.PROJECT) {
      ...
      if (!child || !child.subProject) return null;
      return { ...t, projectId: child.subProject.projectId, subProjectId: child.subProjectId, childProjectId: t.targetId };
    }
  }));

  const validTargets = resolvedTargets.filter((t): t is NonNullable<typeof t> => t !== null);
  if (validTargets.length === 0) return [];

  // 2. Group by Root Project
  const rootGroups = new Map<string, typeof validTargets>();
  for (const rt of validTargets) {
    if (!rootGroups.has(rt.projectId)) rootGroups.set(rt.projectId, []);
  ...
  const finalAllocations: { targetType: TargetType, targetId: string, amount: Prisma.Decimal }[] = [];
  ...
  for (const [projectId, items] of rootGroups.entries()) {
    ...
    for (const item of items) {
      if (item.subProjectId) selectedSubs.add(item.subProjectId);
    }
    ...
    if (selectedSubs.size === 0) {
      ...
      for (const item of items) {
        if (item.subProjectId) {
          if (!subGroups.has(item.subProjectId)) subGroups.set(item.subProjectId, []);
          subGroups.get(item.subProjectId)!.push(item);
        }
      }

      for (const [subId, subItems] of subGroups.entries()) {
        const selectedChilds = new Set<string>();
        for (const item of subItems) {
          if (item.childProjectId) selectedChilds.add(item.childProjectId);
        }
...
export const ledgerService = {
  ...
  async create(data: CreateLedgerEntryDTO) {
    const result = await prisma.$transaction(async (tx) => {
      const workerAdvancesTotal = data.workerAdvances.reduce(
        (sum, wa) => sum.add(new Prisma.Decimal(wa.advanceAmount || 0)),
      ...
      for (const ap of entry.allocatedProjects) {
        const rootId = await projectService.getRootProjectId(ap.targetType, ap.targetId, tx as any);
    ...
    for (const cp of result.completedProjects) {
      ...
      await projectService.checkAndCascadeCompletion(cp.targetType, cp.targetId, prisma as any);
    }
  ...
  async update(id: string, data: UpdateLedgerEntryDTO) {
    ...
    for (const oldCp of entry.completedProjects) {
      if (!newCompletedIds.has(oldCp.targetId)) {
        await projectService.revertCascadeCompletion(oldCp.targetType, oldCp.targetId, prisma as any);
      }
    }
  ...
  async delete(id: string) {
    ...
    for (const cp of entry.allocatedProjects) {
      await projectService.revertCascadeCompletion(cp.targetType, cp.targetId, prisma as any);
    }
