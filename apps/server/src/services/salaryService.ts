...
import { CalculateSalaryDTO, SaveSalaryDTO, UpdateSalaryDTO } from '@sg/types';
...
export const salaryService = {
  ...
  async getHistory(id: string) {
    ...
    const dailyPaidHistory = await Promise.all(ledgerAdvances.map(async (adv) => {
      ...
      if (adv.targetType && adv.targetId) {
        targetTitle = await projectService.resolveTargetName(adv.targetType, adv.targetId, prisma as any);
      }
      return {
  ...
  async calculate(data: CalculateSalaryDTO) {
  ...
  async syncSalary(workerId: string, date: Date) {
